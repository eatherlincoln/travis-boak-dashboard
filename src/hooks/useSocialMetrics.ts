...
async function fetchMetrics(source: 'instagram'|'youtube'|'tiktok') {
  ...
  const out: Metrics = {};
  for (const r of (data ?? [])) out[r.metric] = { value: Number(r.value), updated_at: r.updated_at };
  return out;
}
...
export function useSocialMetrics(source: 'instagram'|'youtube'|'tiktok') {
  ...
  async function load() {
    try {
      ...
      setMetrics(m);
      const latest = Object.values(m).sort((a,b)=> (a.updated_at<b.updated_at?1:-1))[0]?.updated_at || '';
      setUpdatedAt(latest);
      setLoading(false);
    } catch (e:any) {
  ...
  useEffect(() => {
    const ch = supabase
      .channel(`ap_${source}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'analytics_public', filter: `source=eq.${source}` }, () => load())
      .subscribe();
