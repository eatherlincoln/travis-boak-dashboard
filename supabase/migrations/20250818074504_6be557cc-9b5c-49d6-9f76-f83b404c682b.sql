...
CREATE POLICY IF NOT EXISTS "Users can view their own audience data"
ON public.platform_audience
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own audience data"
ON public.platform_audience
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own audience data"
ON public.platform_audience
FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own audience data"
ON public.platform_audience
FOR DELETE
USING (auth.uid() = user_id);
...
-- Also ensure youtube_stats is upserted by edge function; no DB change needed here.